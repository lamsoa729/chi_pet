...
class UnitConversion(object):
    ...
    def __init__(self, yaml_file): 
        with open(yaml_file) as f: yaml_dict = yaml.load(f)
        ...
        for p in conv_params:
            ...
            if prop['units'] == "None":
                ...
                if len(units) != len(dim):
                    raise Exception('Number of units and dimensions do not match in param %s' %k)
    ...
    def WriteYamlFile(self, name=""):
        ...
        if name == "": name = conv_yaml["Type"] + "_units.yaml"
        else: name += "_" + conv_yaml["Type"] + "_units.yaml"
...
# Write function to use conv_yaml to make latex tables
    def WriteLatexTable(self, f_name=""):
        ...
        if f_name == "": f_name = "units_tab.tex"
        else: f_name += "_units_tab.tex"
...
if __name__ == "__main__":
    ...
    elif len(sys.argv) == 3: name = sys.argv[2]
    else: 
